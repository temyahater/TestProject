...
async function deleteFetch(id){
    try {
      const response = await fetch('http://localhost:4000/users/'+String(id), {
        method: 'DELETE'
      });
...
class UserTable extends Component {
    ...
    updateUsers(){
        fetch('http://localhost:4000/users').then(response=>{return response.json();}).then(data =>{this.setState({users: data})});
                ...
                {this.state.users.map(user=>
                    <tr key={user._id}>
                        ...
                        <td>
                            ...
                            Delete</button> */}
                            <button className="btn btn-danger" onClick={()=>window.confirm('Are u sure?')?deleteFetch(user._id)&&this.setState({users: this.state.users.filter(el=>el._id!==user._id)}):0}>
                            Delete</button>
                        </td>
                        </tr>)
                }
